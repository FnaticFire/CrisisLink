...
// Calls Gemini 2.5 Flash-Lite server-side for emergency classification
...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = `You are CrisisLink's emergency triage AI (Gemini 2.5 Flash-Lite). Analyze this distress call and scene.
...
Be decisive. Prioritize life safety. Return ONLY raw JSON.`;
    ...
    return NextResponse.json({ error: 'Could not parse JSON from AI response', raw: content }, { status: 500 });
  } catch (error: any) {
