...
export default function ActiveEmergencyPage() {
  ...
  const [etaTimer, setEtaTimer] = useState(240); // Internal timer to move the mock pin

  // Haversine formula to dynamically calculate distance between two lat/lng points
  ...
  // Dynamic ETA & tracking coordinates
  const currentLat = activeAlert.userLocation.lat + (0.01 * (etaTimer / 240));
  const currentLng = activeAlert.userLocation.lng + (0.01 * (etaTimer / 240));
  
  const distanceKm = calculateDistance(activeAlert.userLocation.lat, activeAlert.userLocation.lng, currentLat, currentLng);
  // Estimate ETA based on 40 km/h constant responder speed
  const dynamicEtaSeconds = Math.max(0, Math.floor((distanceKm / 40) * 3600));
    ...
    <div className="h-[100dvh] w-full flex flex-col bg-black overflow-hidden relative">
      ...
      <div className="h-1/2 w-full relative z-0">
        <MapComponent 
          ...
          trackingPos={[currentLat, currentLng]}
        />
        ...
        <div className="absolute top-6 left-6 z-10 pointer-events-none flex flex-col gap-2">
          ...
          <div className="bg-black/80 backdrop-blur px-4 py-2 rounded-xl border border-white/10 flex items-center gap-2">
            ...
            <span className="text-white font-bold text-xs">Unit arriving in {formatETA(dynamicEtaSeconds)} ({distanceKm.toFixed(1)} km)</span>
          </div>
        </div>
      </div>
